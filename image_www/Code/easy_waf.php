...
<body>
  <nav class="navbar fixed-top navbar-expand-lg navbar-light" style="background-color: #3EA055;">
    <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
      ...
      $input_uname =name_filter($input_uname);
      ...
      //filter 
      function name_filter($input_uname){
	$input_uname= preg_replace('/or/i',"", $input_uname);			//strip out OR (non case sensitive)
	$input_uname= preg_replace('/AND/i',"", $input_uname);		//Strip out AND (non case sensitive)
	$input_uname= preg_replace('/union/i',"", $input_uname);      //Strip out union (non case sensitive)
	$input_uname= preg_replace('/select/i',"", $input_uname); 
	//$input_uname= preg_replace('/#/i',"", $input_uname);
	return $input_uname;
      }


      function passwd_filter($hashed_pwd){
        $id= preg_replace('/or/i',"", $hashed_pwd);                    //strip out OR (non case sensitive)
        $id= preg_replace('/AND/i',"", $hashed_pwd);           //Strip out AND (non case sensitive)
        $id= preg_replace('/union/i',"", $hashed_pwd);      //Strip out union (non case sensitive)
        $id= preg_replace('/select/i',"", $hashed_pwd);
        return $hashed_pwd;}
	  ...
	  echo "<tr>";
          ...
          echo "<ul class='navbar-nav mr-auto mt-2 mt-lg-0' style='padding-left: 30px;'>";
          echo "<li class='nav-item active'>";
          echo "<a class='nav-link' href='#'> <span class='sr-only'>(current)</span></a>";
          echo "</li>";
          echo "<li class='nav-item'>";
    ...
    function logout(){
      location.href = "easy_waf_logout.php";
    }
    </script>
  </body>
  </html>
